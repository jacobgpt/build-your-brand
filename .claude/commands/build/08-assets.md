...
2. Write the brief:
   ...
   ```
   Reads the foundation and folds positioning, audience, tone,
...
5. Build it through the gate:
   ...
   type `y` yourself — that's the actual gate, shown live, not decoration.
   On approve, the real Higgsfield CLI builds it, downloads to
   `records/assets/`, and a dated record writes to `records/runs/`.

6. **Reject one on purpose.** Run step 5 again and this time type anything
   other than `y`. Open the resulting record in `records/runs/` — the
   rejection is on file too, dated, with no asset produced. A record that
   only contains what worked isn't a record, it's a highlight reel.
